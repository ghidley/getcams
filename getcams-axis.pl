...
=begin comment
  ...
  Above parameters read in by run_cameras service (parent script) and passed to this program as command line args:
  ...
  [uncomment S3 and CEPH variables below before testing in isolation]
...
### Axis change ... type not used for axis cams (-c or -m)
#$CAMERA="$CAMERA-$TYPE";
###
...
while (<FILE>) {
    chomp;
    my @elements = split /:/, $_;
    next unless $elements[0] eq $HOST; #CAM name same as host basename in cam_access file
    $FOUND= "1";
    $LOGIN = $elements[1];
    $PWD = $elements[2];
}
